...
PHY_BLOCK INDEX_Format(PHY_BLOCK total_block, PHY_BLOCK fmt_current_block) {
  ...
  for (i = 0; fmt_current_block < total_block; fmt_current_block++, i++) {
    ...
    if (ret == STATUS_SUCCESS) {
      if (index_block == INVALID_BLOCK) {
        ...
        uart_printf("**%s**: index physical block = %d\n", __func__, fmt_current_block);
      } else {
  ...
  uart_printf("**%s**: free blocks total num= %d\n", __func__,free_block_index);
...
STATUS INDEX_Update_Commit() {
  ...
  do {
    if (is_updating_area == TRUE) {
      ...
      if (ret == STATUS_SUCCESS) {
        /* write area block */
        ret = AREA_Update(updating_logical_block, updating_physical_block,
                          updating_block_ec);
...
STATUS INDEX_FreeBlock_Get(DIE_INDEX die, PHY_BLOCK* block, ERASE_COUNT* ec) {
  ...
  do {
    ...
    //直接for循环遍历所有free block后，遇到第一个无效块后，返回i=FREE_BLOCK_COUNT
...
void INDEX_FreeBlock_Put(PHY_BLOCK dirty_block, ERASE_COUNT dirty_block_ec) {
  ...
  /*
   //jsj修改 先擦除一遍块，若是坏块，加入坏块表，
  if (MTD_Erase(dirty_block) != STATUS_SUCCESS) {
    ANCHOR_LogBadBlock(dirty_block);
    (void)ANCHOR_Update();
    return;
  }else{
  */  
   //将物理块加入FBT (按EC递增排序) 
   //由于从free block 往外分配时会先检查是否是坏块，所以不必再此检查是否为坏块  
