...
static STATUS ubi_reclaim_badblock(LOG_BLOCK log_block, PHY_BLOCK phy_block,
                                   ERASE_COUNT phy_block_ec, PAGE_OFF bad_page,
                                   PHY_BLOCK* new_phy_block,
                                   ERASE_COUNT* new_ec);

static UINT32 ubi_find_die_buffer(PHY_BLOCK block);
...
STATUS UBI_Format() {
  ...
  PHY_BLOCK fmt_current_block = DATA_FIRST_BLOCK;//7
  STATUS ret = STATUS_SUCCESS;  
  //jsj增加  
  ANCHOR_Format_Clean();//先擦除6个anchor 块
  /* try to read out only the anchor table first for bad block table */
  ret = ANCHOR_Init();
  //擦除6个anchor 块后将导致每次上电都读anchor table失败
  //然后会重新扫描所有物理块，建立坏块表
  if (ret != STATUS_SUCCESS) {
    int bad_blk=0;
    PHY_BLOCK block;
    uart_printf("******UBI_Format():Blocks Layout****\r\n");
    ...
    for (block = 0; block < CFG_LOG_BLOCK_COUNT; block++) {
      ...
      if (ret != STATUS_SUCCESS) {
        uart_printf("%s: find a bad block: %d\r\n", __func__,block);
        ANCHOR_LogBadBlock(block);
      }
      */
      ret = MTD_Erase(block);
      if (ret != STATUS_BADBLOCK) 
      {
        ret = MTD_CheckBlock(block);        
      }
      if (ret == STATUS_BADBLOCK)
      {
        bad_blk++;
        uart_printf("%s: find a bad block: %d.Bad block - %u\n", __func__, bad_blk, block);
        ANCHOR_LogBadBlock(block);
      }      
    }
  }  
  ...
  do {
    ...
    remaining_count = (DATA_BLOCK_COUNT+DATA_FIRST_BLOCK) - fmt_current_block;
    block_count = MIN(remaining_count, CFG_PHY_BLOCK_PER_AREA); 
    
    //uart_printf("%s: area=%d,fmt_current_block=%d,block_count=%d\r\n",__func__,area,fmt_current_block,block_count);
  ...
  uart_printf("total_data_block=%d\r\n",total_data_block);
    

  if (fmt_current_block != INVALID_BLOCK) {
    //jsj增加  保证Index block是好块  
    while (ANCHOR_IsBadBlock(fmt_current_block) == TRUE) {
      fmt_current_block++;
    }
    /* formatting index */
    //uart_printf("%s Index PEB = %d\n", __func__, fmt_current_block);
...
STATUS UBI_Init() {
  ...
  if (ret == STATUS_SUCCESS) {
    ...
    ret = INDEX_Init(&updating_logical_block, &updating_origin_block,
                     &updating_block_ec);
  }
    ...
    for (area = 0; area < AREA_COUNT; area++) {
      ...
      if (ret == STATUS_SUCCESS) {
        if (area == updating_area) {
          ...
          uart_printf("**%s**:updating_area=%d\r\n", __func__,updating_area);          
          if (AREA_CheckUpdatePLR(updating_logical_block, updating_origin_block,updating_block_ec) == TRUE) {
            /* continue to update the area table */
            INDEX_Update_AreaUpdate(updating_logical_block, updating_origin_block, updating_block_ec);
...
STATUS UBI_Write(LOG_BLOCK block, PAGE_OFF page, void* buffer, SPARE spare,BOOL async) {
  ...
  while (ret == STATUS_BADBLOCK) {
    ...
    ret = ubi_reclaim_badblock(dice_hold[die_index].log_block,
                               ...
                               dice_hold[die_index].page, &new_phy_block,
                               &new_ec);
    if (ret == STATUS_SUCCESS && dice_hold[die_index].phy_block != INVALID_BLOCK) {
      dice_hold[die_index].phy_block = new_phy_block;
      dice_hold[die_index].ec = new_ec;
      /* write last page in die buffer */
      //jsj 下面写的应是坏块中坏页的内容
      ret = MTD_Program(new_phy_block, dice_hold[die_index].page,
  ...
  if (ret == STATUS_SUCCESS && page != INVALID_PAGE) {
    ...
    if (async == FALSE) {
      ret = MTD_WaitReady(phy_block);
      while (ret == STATUS_BADBLOCK) {
        ret = ubi_reclaim_badblock(block, phy_block, phy_block_ec, page,
                                   &new_phy_block, &new_ec);
        if (ret == STATUS_SUCCESS) {
          /* write last page in die buffer */
        ...
        if (ret == STATUS_SUCCESS) {
          ret = MTD_WaitReady(new_phy_block);
          // Bo: I suspect the following while loop has some bugs when the SSD needs to reclaim bad blocks mutiple times. 
          // a correct way should update the old phy_block with the new_phy_block during each loop
          // the following lines added by Bo to fix the bug
          phy_block = new_phy_block;
          phy_block_ec = new_ec;
        }
      }
...
STATUS UBI_Flush() {
  ...
  for (i = 0; i < TOTAL_DIE_COUNT; i++) {
    if (ret == STATUS_SUCCESS) {
      ...
      ret = UBI_Write(dice_hold[i].log_block, INVALID_PAGE, NULL, NULL, TRUE);
    }
  }
...
STATUS UBI_Erase(LOG_BLOCK block, LOG_BLOCK die_index) {
  STATUS ret = STATUS_SUCCESS;
  UINT32 die = die_index % TOTAL_DIE_COUNT;//只有一个die，die=0
  PHY_BLOCK phy_block = INVALID_BLOCK;
  ERASE_COUNT ec = INVALID_EC;
  static UINT32 last_position = 0;
...
STATUS UBI_SWL() {
  ...
  //用（N:空闲块表擦除次数最大的块）替换掉（M:area正使用块中擦除次数最小的块）
  //因为M中的数据一般就是cold数据，可以转存到擦除较大的块中。  
  min_block_offset = AREA_FindMinECBlock(anchor_table.swl_current_area,
                                         &min_physical_block, &min_block_ec);
  INDEX_FreeBlock_GetMaxECBlock(&max_physical_block, &max_block_ec);

  /* check if SWL is required */
  if (max_physical_block != min_physical_block&& max_physical_block != INVALID_BLOCK
      && min_physical_block != INVALID_BLOCK && max_block_ec != INVALID_EC
      && min_block_ec != INVALID_EC && max_block_ec > min_block_ec
    ...
    if (ret == STATUS_SUCCESS) {
      ...
      logical_block = ((anchor_table.swl_current_area) * CFG_PHY_BLOCK_PER_AREA) + min_block_offset;
      INDEX_Update_AreaUpdate(logical_block, max_physical_block, max_block_ec);
    ...
    anchor_table.swl_current_area = (anchor_table.swl_current_area + 1) % AREA_COUNT;
  }
  return ret;
}
