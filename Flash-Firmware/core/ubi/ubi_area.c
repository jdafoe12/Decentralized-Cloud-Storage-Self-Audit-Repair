...
static AREA_BLOCK cached_area_table[CFG_PHY_BLOCK_PER_AREA];//CFG_PHY_BLOCK_PER_AREA=256
...
PHY_BLOCK AREA_Format(PHY_BLOCK block_count, PHY_BLOCK fmt_current_block,
  ...
  if (ret == STATUS_SUCCESS) {
    ...
    //uart_printf("%s: write %d area table to %d phy block\r\n",__func__,area,fmt_current_block);
  }
...
PHY_BLOCK AREA_GetBlock(LOG_BLOCK logical_block) {
  ...
  block = BLOCK_OFFSET_AREA(logical_block); // block identifes where is the entry of the mapping from the corresponding area table
  area = AREA_INDEX(logical_block); // area identifies where is the corresponding area table
