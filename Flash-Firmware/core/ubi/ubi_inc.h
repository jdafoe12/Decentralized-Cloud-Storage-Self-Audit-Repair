...
#define FREE_BLOCK_COUNT            \
      ((MPP_SIZE/4-sizeof(AREA_UPDATE_PLR))/sizeof(PHY_BLOCK))//125

#define REAL_FREE_BLOCK_COUNT       \
      (CFG_LOG_BLOCK_COUNT/(100/(100-GOOD_BLOCK_PERCENT)))//81

#define ANCHOR_BLOCK_COUNT          \
      (CFG_LOG_BLOCK_COUNT*2*2/PAGE_PER_PHY_BLOCK/PAGE_PER_PHY_BLOCK + 2)//6

#define ANCHOR_BADBLOCK_COUNT       (MPP_SIZE/sizeof(PHY_BLOCK)-11)//501
...
#define ANCHOR_FIRST_BLOCK          (CODE_BLOCK+CODE_BLOCK_COUNT)//1
#define ANCHOR_LAST_BLOCK           (ANCHOR_FIRST_BLOCK+ANCHOR_BLOCK_COUNT-1)//6

#define INDEX_BLOCK_COUNT           (1)

#define DATA_FIRST_BLOCK            (ANCHOR_FIRST_BLOCK+ANCHOR_BLOCK_COUNT)//7
#define DATA_BLOCK_COUNT            (CFG_LOG_BLOCK_COUNT -  \
                                     CODE_BLOCK_COUNT -     \
                                     ANCHOR_BLOCK_COUNT -   \
                                     INDEX_BLOCK_COUNT -    \
                                     REAL_FREE_BLOCK_COUNT)//4007
...
static void printint(int xx, int base, int sign)
{
  ...
  while(--i >= 0)
    UartWrite((unsigned char *)&buf[i], sizeof(unsigned char));
}
static void printuint(unsigned int xx, int base)
{
  static char digits[] = "0123456789abcdef";
  char buf[32];
  int i;
...
static void uart_printf(char *fmt, ...)
{
  ...
  for(i = 0; (c = fmt[i] & 0xff) != 0; i++)
  {
    ...
    {
      if(c == '\n')
        UartWrite((unsigned char *)&cr, sizeof(unsigned char));
      UartWrite((unsigned char *)&c, sizeof(unsigned char));
      continue;
    }
    c = fmt[++i] & 0xff;
    if(c == 0)
    ...
    case 's':
      if((s = (char*)*argp++) == 0)
        s = "(null)";
      for(; *s; s++)
        UartWrite((unsigned char *)s, sizeof(unsigned char));
      break;
    case '%':
      UartWrite((unsigned char *)&pcent, sizeof(unsigned char));
      break;
    default:
      //Print unknown % sequence to draw attention.
      //UartWrite((unsigned char *)'%', sizeof(unsigned char));
      UartWrite((unsigned char *)&c, sizeof(unsigned char));
      break;
    }
  }
}
#define DBG uart_printf("****** %s @ %d*******\n", __func__, __LINE__)
//jsj 增加调试功能结束
 ...
 *
 ***************************************************/
void INDEX_Update_AreaUpdate(PHY_BLOCK logical_block, PHY_BLOCK physical_block,
                             ERASE_COUNT block_ec);
...
//jsj 添加 开始
/***************************************************
 * Funcion Name: ANCHOR_Format_Clean
 *
 * Description:
 *    Formats all anchor blocks
 *
 * Return Value:
 *
 * Parameter List:
