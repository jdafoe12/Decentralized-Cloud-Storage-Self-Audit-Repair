...
BOOL HDI_IsHotPage(PGADDR addr) {
  ...
  for (i = 0; i < HDI_FUNC_COUNT; i++) {//4
    ...
    if (*hot_value < HDI_HOT_DATA_THERSHOLD) {//0x60=96
      ret = FALSE;
    }
  }

  /* cold down the hash table after every XXX times of access */
  if (count++ == HDI_COLDDOWN_DELAY) {//4096
    for (i = 0; i < MPP_SIZE; i++) {
      hdi_hash_table[i] >>= 1;
    }
    count = 0;
...
STATUS HDI_Commit() {
  ...
  if (hdi_current_page == PAGE_PER_PHY_BLOCK) {
    ...
    next_block = hdi_current_block ^ 1;//即逻辑II块4和5，交替使用
  ...
  if (ret == STATUS_SUCCESS) {
    ret = UBI_Write(hdi_current_block, hdi_current_page, hdi_hash_table, NULL,FALSE);
  }

  if (ret == STATUS_SUCCESS) {
    PM_NODE_SET_BLOCKPAGE(root_table.hdi_current_journal, hdi_current_block,
                          hdi_current_page);
    hdi_current_page++;
  }
