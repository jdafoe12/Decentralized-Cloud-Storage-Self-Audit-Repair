...
STATUS DATA_Format() {
  UINT32 i;
  LOG_BLOCK block = DATA_START_BLOCK;//46
    ...
    if (total_valid_page != 0) {
            ...
            if (ret == STATUS_SUCCESS) {
              if (true_block == dirty_block && true_page == page) {//若是valid页
                ...
                if (ret == STATUS_SUCCESS) {
                  ...
                  ret = UBI_Write(reclaim_block, reclaim_page, data_buffer,
                                  spare, FALSE);
                }

                if (ret == STATUS_SUCCESS) {
                  /* update pmt */
                  ret = PMT_Update(pages_buffer[page][0], reclaim_block,
                                   reclaim_page);
      ...
      for (j = 0; j < JOURNAL_BLOCK_COUNT; j++) {
        ...
        if (ret == STATUS_SUCCESS) {
          ...
          PM_NODE_SET_BLOCKPAGE(root_table.reclaim_journal[j], dirty_blocks[j],0);
...
STATUS DATA_Replay(JOURNAL_ADDR* journals) {
  ...
  while (journal_edition != MAX_UINT32) {
    for (j_index = 0; j_index < JOURNAL_BLOCK_COUNT; j_index++) {//1
  ...
  for (j_index = 0; j_index < JOURNAL_BLOCK_COUNT; j_index++) {//1
    ...
    for (page = 0; page < PAGE_PER_PHY_BLOCK; page++) {
      ...
      if (ret == STATUS_SUCCESS) {
        ...
        /* write meta data to last page */
        ret = UBI_Write(block, PAGE_PER_PHY_BLOCK - 1, meta_data_buffer, NULL,FALSE);
      } else {
        ret = STATUS_SUCCESS;
        break;
      }
