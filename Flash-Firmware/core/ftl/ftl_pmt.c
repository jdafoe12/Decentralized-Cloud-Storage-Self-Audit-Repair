...
STATUS PMT_Format() {
  ...
  UINT32 i;
  UINT32 j;
  UINT32 pmt_cluster_count = ((FTL_Capacity() + PM_PER_NODE - 1) /  PM_PER_NODE);//471
  ...
  if (ret == STATUS_SUCCESS) {
    ...
    block_dirty_table[pmt_block] = 0;
    block_dirty_table[pmt_block + 1] = 0;//为什么设置这个块??
  }
...
STATUS PMT_Update(PGADDR page_addr, LOG_BLOCK block, PAGE_OFF page) {
  ...
  if (ret == STATUS_SUCCESS) {
    ...
    if (block != INVALID_BLOCK) {
      ...
      PM_NODE_SET_BLOCKPAGE(cluster_addr[PAGE_IN_CLUSTER(page_addr)], block, page);
    } else {
